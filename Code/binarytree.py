...
class BinaryTreeNode(object):
    ...
    def is_leaf(self):
        ...
        if self.left or self.right:  # if has no children...
            return False
        return True
    ...
    def height(self):
        ...
        # Check if left and right childen have a values, if so calculate height
...
class BinarySearchTree(object):
    ...
    def height(self):
        ...
        TODO: Best and worst case running time: ??? under what conditions?"""
        # Check if root node has value, if so calculate height
    ...
    def search(self, item):
        ...
        # Find a node with the given item, if any
        node = self._find_node_recursive(item, self.root)
        # Return node's data if found, or None
        if node is not None:
            return node.data
        return None
    ...
    def insert(self, item):
        ...
        # Find parent node where the given item should be inserted
        parent = self._find_parent_node_recursive(item, self.root)
        if parent is None and self.root.data:
            parent = self.root
        # Check if the given item should be inserted left/right of parent node
    ...
    def _find_node_iterative(self, item):
        ...
        node = self.root  # start w/ root node
        ...
        while node is not None:
            if item == node.data:  # Check if  given item matches node's data
                ...
                node = node.left  # descend to node's left child
    ...
    def _find_node_recursive(self, item, node):
        ...
        elif item == node.data:  # Check if item matches the node's data
            return node  # Return the found node

        # Check  item is less than the node's data
        elif item < node.data:
            # recursively descend to node's left child if it exists
            return self._find_node_recursive(item, node.left)

        # Check if given item greater than node's data
        elif item > node.data:
    ...
    def _find_parent_node_iterative(self, item):
        ...
        # Loop until we descend past the closest leaf node
        while node is not None:
            # Check if the given item matches node's data
            if node.data == item:
                return parent  # Return parent of found node

            # check if the  item is less than node's data
            elif item < node.data:
                # Update the parent and descend to the node's left child
                parent = node
                node = node.left

            # check if the  item is greater than node's data
            elif item > node.data:
                parent = node
                node = node.right
